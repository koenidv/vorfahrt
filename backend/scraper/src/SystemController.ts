...
export class SystemController {
  ...
  startService(serviceId: string): this | Error {
    ...
    if (!observed) {
      console.warn(
        clc.bgRedBright("SystemController"),
        clc.red(`Tried to start unknown scraper ${serviceId}`)
      )
  ...
  stopService(serviceId: string): this | Error {
    ...
    if (!observed) {
      console.warn(
        clc.bgRedBright("SystemController"),
        clc.red(`Tried to stop unknown scraper ${serviceId}`)
      )
      return new Error(`No scraper with id ${serviceId} registered`)
    }
  ...
  setServiceSpeed(serviceId: string, cyclesMinute: number): this | Error {
    ...
    if (!observed) {
      console.warn(
        clc.bgRedBright("SystemController"),
        clc.red(`Tried to set speed of unknown scraper ${serviceId}`)
      )
      return new Error(`No scraper with id ${serviceId} registered`)
    }
